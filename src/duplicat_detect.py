...
#            log_entry["hash"] = file_hash
            ...
            if not core.is_duplicate(file_hash):
                print(f"♻️ 发现重复文件: {file_name} -> 移至重复区")
                target_path = os.path.join(tar_dir, file_name)
#                os.makedirs(DUPLICATE_DIR, exist_ok=True)
                shutil.copy(file_path, tar_dir)
